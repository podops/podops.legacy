...
func ProductionEndpoint(c echo.Context) error {
	...
	if !a.ValidResourceName(showName) {
		return ErrorResponse(c, http.StatusBadRequest, fmt.Errorf("invalid name '%s'", showName))
	}
	...
	return StandardResponse(c, http.StatusCreated, p)
}
...
func ListProductionsEndpoint(c echo.Context) error {
	...
	return StandardResponse(c, http.StatusOK, &a.ProductionList{Productions: productions})
}
