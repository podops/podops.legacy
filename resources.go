...
func DefaultShow(name, title, summary, guid, portal, cdn string) *Show {
	return &Show{
		...
		Image: Asset{
			URI: fmt.Sprintf("%s/default/cover.png", cdn), // FIXME create a real resource entry to be consistent
			Rel: "external",
		},
	}
}
...
func DefaultEpisode(name, parentName, guid, parent, portal, cdn string) *Episode {
	return &Episode{
		...
		Image: Asset{
			URI: fmt.Sprintf("%s/default/episode.png", cdn), // FIXME create a real resource entry to be consistent
			Rel: "external",
		},
		Enclosure: Asset{
