...
func CreateAuthorizationEndpointLegacy(c echo.Context) error {
	var req *a.OldAuthorizationRequest = new(a.OldAuthorizationRequest)
...
func ValidateAuthorizationEndpoint(c echo.Context) error {
	...
	auth, err := FindAuthorizationByToken(appengine.NewContext(c.Request()), token)
	if auth == nil || err != nil {
		return c.NoContent(http.StatusUnauthorized)
	}
	if !auth.IsValid() {
